...
plt.figure(figsize=(10, 10))
sns.barplot(
    data=summary_df.sort_values("Pearson", ascending=False),
    x="Pearson", y="cell_type", palette="coolwarm", hue="cell_type", legend=False
)
...
for i, row in enumerate(top_n.itertuples()):
    df = pd.read_csv(os.path.join(PREDICTIONS_FOLDER, f"predictions_{row.cell_type}.csv"))
